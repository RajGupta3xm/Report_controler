...
class OrderController extends Controller {
   ...
   public function show(Request $request, $id = null) {
     ...
     if($getUserSubscriptionId){
        ...
        //   $getdate = UserProfile::select('id','subscription_id','diet_plan_type_id')->where('user_id',$id)->first();
        //  if($getdate){
        //     $datee =[];
        ...
        //      $getSchedule = SubscriptionMealVariantDefaultMeal::join('meal_schedules','subscription_meal_plan_variant_default_meal.meal_schedule_id','=','meal_schedules.id')
        //    ->select('meal_schedules.name','subscription_meal_plan_variant_default_meal.date')
        //    ->where('subscription_meal_plan_variant_default_meal.meal_plan_id',$getdate->subscription_id)
        //    ->where('subscription_meal_plan_variant_default_meal.date',$ddd)
        //    ->get();
    ...
    if(!empty($customCalorie))
   {
    ...
    if(UserChangeDeliveryLocation::where('user_id',$id)->where('subscription_plan_id',$checkPlan->subscription_id)->where('change_location_for_date',$dates)->exists()){
        ...
        if(UserSkipTimeSlot::where('user_id',$id)->where('subscription_plan_id',$checkPlan->subscription_id)->where('skip_date',$dates)->exists()){
      ...
      if(UserSkipDelivery::where('user_id',$id)->where('subscription_plan_id',$checkPlan->subscription_id)->where('skip_delivery_date',$dates)->exists())
      {
